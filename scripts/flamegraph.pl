#!/usr/bin/perl -w
#
# Original source: https://github.com/brendangregg/FlameGraph/blob/master/flamegraph.pl
...
USAGE: $0 [options] infile > outfile.svg\n
	...
	--colors PALETTE # set color palette. choices are: hot (default), mem,
	                 ...
	                 # aqua, yellow, purple, orange, fault
...
# background colors:
# - yellow gradient: default (hot, java, js, perl, fault)
# - green gradient: mem
...
sub color {
	...
	if (defined $type and $type eq "fault") {
		# Handle both annotations (_[j], _[i], ...; which are
		# accurate), as well as input that lacks any annotations, as
		# best as possible.
		if ($name =~ m:\[r\]$:) {	# read fault
			$type = "aqua";
		} elsif ($name =~ m:\[w\]$:) {	# write fault
			$type = "red";
		} elsif ($name =~ m:\[p\]$:) {	#  writeprotect fault
			$type = "yellow";
		} else {	# default
			$type = "orange";
		}
