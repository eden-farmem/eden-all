...
-s2,  --suffix2   \t a plain suffix defining the second group of runs\n
-cs2, --csuffix2  \t same as -s2 but a more complex one (with regexp pattern)\n
-l2,  --label2    \t plot label for second group of runs\n
-s3,  --suffix3   \t a plain suffix defining the third group of runs (optional)\n
-cs3, --csuffix3  \t same as -s3 but a more complex one (with regexp pattern)\n
-l3,  --label3    \t plot label for third group of runs\n
...
if [[ $FORCE ]] || [ ! -f "$plotname" ]; then
    python3 ${SCRIPT_DIR}/plot.py ${plots}                          \
        ...
        -xc $XCOL -xl "$XLABEL"                               \
         --size 4.5 3 -fs 11 -of $PLOTEXT -o $plotname -lt "Madvise Notif"
...
if [[ $FORCE ]] || [ ! -f "$plotname" ]; then
    # sed '/NoKona/d' $datafile > $datafile_kona      #remove nokona run
    python3 ${SCRIPT_DIR}/plot.py ${plots}                          \
        -yc $ycol -ls solid -yl "$ydesc" --ymin 0 --ymul 1e-9       \
        -xc $XCOL -xl "$XLABEL"                               \
         --size 4.5 3 -fs 11 -of $PLOTEXT -o $plotname -lt "Madvise Notif"
    if [[ $DISPLAY_EACH ]]; then display $plotname &    fi
fi
